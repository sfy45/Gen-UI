...
async function generateResponse(query: string): Promise<string> {
  ...
  if (lowerQuery.startsWith("weather in ") || lowerQuery === "weather") {
    const city = lowerQuery.startsWith("weather in ")
      ? lowerQuery.substring("weather in ".length).trim()
      : "your current location" // You might want to implement location detection or ask
    if (city === "your current location" && lowerQuery === "weather")
      return "Sure, which city's weather are you interested in? For example, 'weather in London'."
